#!/usr/bin/env python3
"""
Screenshot Reset Scheduler for Youth Secure Check-in
Automatically resets the screenshot database at specified intervals
"""
...
def reset_database():
    """Reset the database by running the seed script"""
    print(f"[{datetime.now()}] Starting database reset...")
    
    try:
        # Run the seed script
        result = subprocess.run(
            [sys.executable, 'screenshot_seed.py'],
            capture_output=True,
            text=True,
            timeout=60
        )
...
def main():
    """Main scheduler loop"""
    print("=" * 60)
    print("Youth Secure Check-in - Screenshot Reset Scheduler")
